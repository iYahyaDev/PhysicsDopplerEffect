...
Doppler Lab is an offline ASP.NET Web Forms educational website for presenting the Doppler effect in a high-school physics lesson or Zoom presentation. It uses C# for page setup, configuration, localization, quiz data, and reference physics checks, while the real-time simulation, audio, charts, pointer interaction, and animation run in client-side JavaScript.
...
- `Presenter.aspx`: guided 16:9 Zoom presentation flow.
- `PhysicsValidation.aspx`: C# and JavaScript reference physics tests.
- `Models`: settings, vector, quiz, and validation models.
- `Services`: localization, quiz repository, physics calculator, and JSON boot payloads.
- `Scripts`: simulation engine, audio, renderer, charts, UI, application modules, quiz, presenter, validation, and localization.
- `Content`: main and presenter CSS.
- `Assets`: original offline SVG illustrations plus supplied motorcycle and listener PNG sprites.
...
- Open `Presenter.aspx` for the guided 16:9 flow.
- Open `Lab.aspx` in another tab when live interaction is needed.
- In Zoom, enable `Share Computer Sound` before using audio.
- Use fullscreen presenter mode for readable values.
- Keyboard shortcuts in the lab: Space play/pause, `R` reset, `F` freeze, `A` linear pass, `C` circular motion, `W` wavefronts, `M` mute, `1`/`2` listener selection.
